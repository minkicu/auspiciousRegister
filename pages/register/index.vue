    ...
    <div class="homepage-wrapper">
        <v-container class="pt-0 pb-0">
            <v-row>
                ...
                <v-col cols="12">
                    <v-form>
                        <v-text-field class="mb-5" color="#FFFFFF" dark v-model="form.firstname"
                            hint="ไม่ต้องระบุคำนำหน้า" label="ชื่อ" :rules="[rules.required]"></v-text-field>
                        <v-text-field class="mb-5" color="#FFFFFF" dark v-model="form.lastname" dense label="สกุล"
                            :rules="[rules.required]"></v-text-field>
                        <div class="gender-group d-flex mt-5">
                            <p>เพศ</p>
                            <div class="circle" :class="form.gender == 1 ? 'active' : ''" @click="chooseGender(1)">
                                <svg viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" class="icon-70">
                                    <rect fill="none" height="256" width="256" />
                                    <circle cx="128" cy="96" fill="none" r="72" stroke="#FFFFFF" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-width="12" />
                                    <line fill="none" stroke="#FFFFFF" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="12" x1="128" x2="128" y1="168" y2="240" />
                                    <line fill="none" stroke="#FFFFFF" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="12" x1="88" x2="168" y1="208" y2="208" />
                                </svg>
                            </div>
                            <p>หญิง</p>
                            <div class="circle" :class="form.gender == 2 ? 'active' : ''" @click="chooseGender(2)">
                                <svg viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" class="icon-70">
                                    <rect fill="none" height="256" width="256" />
                                    <circle cx="104" cy="152" fill="none" r="72" stroke="#FFFFFF" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-width="12" />
                                    <line fill="none" stroke="#FFFFFF" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="12" x1="154.9" x2="216" y1="101.1" y2="40" />
                                    <polyline fill="none" points="168 40 216 40 216 88" stroke="#FFFFFF"
                                        stroke-linecap="round" stroke-linejoin="round" stroke-width="12" />
                                </svg>
                            </div>
                            <p>ชาย</p>
                        </div>
                        <button type="button" class="button" @click="next()">ถัดไป</button>
                    </v-form>
                </v-col>
            </v-row>
        </v-container>
...
export default {
    ...
    methods: {
        isDone() {
            this.$axios
                .get(
                    `https://tellme-340313-default-rtdb.asia-southeast1.firebasedatabase.app/member/${this.$store.getters.getLine.userId}/profile.json`
                )
                .then((res) => {
                    if (res.data != null) {
